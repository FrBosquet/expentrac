...
export const Month = ({ className }: Props) => {
  const { totalFee, alreadyPaidFee } = useSummary()
    ...
    <Card className={twMerge('flex flex-col', className)}>
      ...
      <CardContent>
        <div className="flex flex-col gap-3 text-center">
          ...
          <p className="text-2xl font-semibold text-expentrac-800">
            {euroFormatter.format(alreadyPaidFee)}/<br />
            {euroFormatter.format(totalFee)}
          </p>
          <Progress value={percentage} />
          <p className="text-sm text-theme-light">
            You already paid {percentage.toFixed(0)}% or your monthly fees
          </p>
          <p className="text-sm text-theme-accent">
            {euroFormatter.format(totalFee - alreadyPaidFee)} left
          </p>
        </div>
      </CardContent>
    </Card>
